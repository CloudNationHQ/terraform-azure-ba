...
resource "azurerm_batch_account" "this" {
  ...
  encryption = var.batch.encryption != null ? [{
    key_vault_key_id = var.batch.encryption.key_vault_key_id
  }] : null
...
# batch certificate (deprecated upstream: azure is retiring batch account certificates; retained per module contract)
  ...
  dynamic "container_configuration" {
    ...
    content {
      ...
      container_registries = container_configuration.value.container_registries != null ? [
        for registry in container_configuration.value.container_registries : {
...
resource "azurerm_batch_job" "this" {
  for_each = merge([
    for pkey, pool in(var.batch.pools != null ? var.batch.pools : {}) : {
      for jkey, job in(pool.jobs != null ? pool.jobs : {}) :
      "${pkey}-${jkey}" => {
        pool_key = pkey
        job_key  = jkey
        config   = job
  ...
  name                          = coalesce(each.value.config.name, replace(each.value.job_key, "_", "-"))
