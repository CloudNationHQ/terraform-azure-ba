...
# batch certificate
# note: azurerm_batch_certificate is deprecated upstream but is part of this module's required
# contract, so it is retained. azure derives the certificate name from the thumbprint, so the
# map key is logical only and the resource name argument is not set.
...
resource "azurerm_batch_job" "this" {
  ...
  name                          = coalesce(each.value.job.name, each.value.job_key)
  batch_pool_id                 = azurerm_batch_pool.this[each.value.pool_key].id
  display_name                  = each.value.job.display_name
  priority                      = each.value.job.priority
  task_retry_maximum            = each.value.job.task_retry_maximum
  common_environment_properties = each.value.job.common_environment_properties
}
